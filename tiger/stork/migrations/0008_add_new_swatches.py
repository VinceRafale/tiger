...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for theme in orm.Theme.objects.all():
            stork = Stork(theme)
            for existing, newlist in swatch_map.items():
                instance = stork[existing].instance
                try:
                    assert instance
                except:
                    break
                for picker in newlist:
                   orm.Swatch.objects.create(theme=theme, color=instance.color, alpha=instance.alpha, component=picker) 
